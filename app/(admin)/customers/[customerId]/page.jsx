...
const CustomerDetailPage = async ({ params }) => {
  ...
  return (
    <div className="p-4 md:p-8 text-brand-light">
      <Link href="/customers" className="inline-flex items-center text-gray-400 hover:text-brand-light mb-8">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <Link href={`/customers/${customer.id}/vehicles/new`} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded text-sm inline-flex items-center">
    ...
    <Link href={`/vehicles/${vehicle.id}`} className="text-blue-400 hover:text-blue-300 text-sm font-semibold">
      Ver Historial
    </Link>
    {/* --- NUEVO LINK DE EDITAR --- */}
    <Link href={`/edit/vehicle/${vehicle.id}`} className="text-yellow-400 hover:text-yellow-300 text-sm font-semibold">
      Editar
    </Link>
  </div>
              </li>
